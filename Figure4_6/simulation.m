function [DATA_AIDS,DATA_STRAIN,DATA_DIST] = simulation(ad,tau,y0,IC,~,ML_C,wwa,PK,IC50,DT)
N=2^17;
TF=20*365;
% F=zeros(73,3*N+2);
% rt=0.6;
...
% t=0:TF;
% 
% T=F(:,1);    % HealthF (uninfected) cells 
% Ti=F(:,2:N+1);   %Infected cells (Active)
% Mi=F(:,N+3:2*N+2);   %Infected Macrophages
V=Y0(1,2*N+3:3*N+2);    %Virus 
% M=F(:,N+2);
[mts,ps]=sort(V,'descend');
DATA_AIDS(da,db)=TAIDS;
DATA_STRAIN{da,db}=ps(1:length(SS));
DATA_DIST{da,db}=mts(1:length(SS));

% name=sprintf('VERI_of_COMB_%0.1d_%0.1d',da,db);
% save(name);
clear D S F V Ti T Mi M CPP
            else
DATA_AIDS(da,db)=3000;
DATA_STRAIN{da,db}=[];
DATA_DIST{da,db}=[];
            end
        end
